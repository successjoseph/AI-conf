...
GROQ_API_KEY = "YOUR_GROQ_API_KEY_HERE" # Drop your key here
os.environ["GROQ_API_KEY"] = GROQ_API_KEY
client = Groq(api_key=os.environ.get("GROQ_API_KEY"))

# --- THE TAG TEAM ---
LEAD_DEV = "llama-3.3-70b-versatile" # Now running on Groq
ARCHIVIST = "llama3.2:1b" # running locally on Ollama
...
def chat_with_agent(model_name, history, color):
    """Streams the response. Routes Lead Dev to Groq, and Archivist to local Ollama."""
    messages = [{"role": "system", "content": personas[model_name]}] + history
    full_reply = ""
    print(color, end="") # Turn on agent color
    
    if model_name == LEAD_DEV:
        # Route to Groq API
